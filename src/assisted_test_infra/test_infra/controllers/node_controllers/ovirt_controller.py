...
from assisted_test_infra.test_infra.helper_classes.config.ovirt_config import OVirtControllerConfig
...
class OVirtController(NodeController):
    def __init__(self, config: OVirtControllerConfig, cluster_config: BaseClusterConfig):
    ...
    def list_nodes(self) -> List[Node]:
        ...
        def ovirt_vm_to_node(terraform_vm_state):
            return Node(
        ...
        return list(map(ovirt_vm_to_node, vms))
    ...
    def __get_vms(self) -> List[Dict[str, Any]]:
        vms_object_type = self._tf.get_resources(resource_type="ovirt_virtual_machine")
    ...
    def __run_on_vm(self, name: str, folder, action: Callable) -> None:
        connection = SmartConnect(
            host=self._config.ovirt_fqdn,
            user=self._config.ovirt_username,
            pwd=self._config.ovirt_password,
